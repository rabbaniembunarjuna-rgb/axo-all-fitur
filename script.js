...
function updateLiquidIndicator() {
    const activeLi = document.querySelector('.liquid-list.active');
    if (!activeLi || !liquidIndicator) return;
    ...
    const indicatorWidth = Math.min(44, navWidth * 0.09);
    
    liquidIndicator.style.width = indicatorWidth + 'px';
    liquidIndicator.style.left = (centerX - indicatorWidth / 2) + 'px';
}
...
liquidLists.forEach(li => {
    li.addEventListener('click', function() {
        ...
        if (page && page !== currentLiquidPage) {
            // Kalo page 'about', tampilkan about container
            if (page === 'about') {
            ...
            // Kalo page lain, tampilkan pageContainer
